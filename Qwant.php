<?php
    /**
     * Content scraper for Qwant.com
     *
     * @author SSES Mike
     * @label  Qwant
     */
    class Content_Adapter_Qwant extends Content_Adapter_DeepLinkingAbstract {
        /**
         * Fetch content
         *
        ...
        public function fetchContent($keyword) {
            $this->getClient ()->setHeaders ( 'Referer', 'https://www.qwant.com/' )
            ->setUri ( 'https://www.qwant.com/?q=' )->setParameterGet ( array (
                                                                                      'q'    => urlencode($keyword),
                                                                                      't'   => 'web',
                                                                                      ));
...
if (preg_match_all('/<span class="result__url__long"?>(.*?)<\\/div>/i', $request->getBody(), $result, PREG_PATTERN_ORDER))
{
