<?php
/**
 * Content scraper for bing.com
 *
 * @author SSES Mike
 * @label  Bing
 */
class Content_Adapter_Bing extends Content_Adapter_DeepLinkingAbstract
{
    /**
     * Fetch content
     *
    ...
    {
        $this->getClient()
        ->setHeaders('Referer', 'http://www.bing.com/')
        ->setUri('http://www.bing.com/search')
        ...
        switch($this->getOption('scrape_source'))
        {
            case content::SCRAPE_SOURCE_CRAWL:
                ...
                if (preg_match_all('/<li\\s*class="b_algo"><h2><a href="(.*?)"/is', $request->getBody(), $result, PREG_PATTERN_ORDER))
                {
