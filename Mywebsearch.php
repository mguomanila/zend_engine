<?php
/**
 * Content scraper for https://search.mysearch.com
 *
 * @author SSES Mike
 * @label  MyWebSearch
 */
class Content_Adapter_Mywebsearch extends Content_Adapter_DeepLinkingAbstract {
	/**
	 * Fetch content
	 *
	...
	public function fetchContent($keyword) {
		$this->getClient ()->setHeaders ( 'Referer', 'https://search.mysearch.com/' )
			->setUri ( 'https://search.mysearch.com/web' )->setParameterGet ( array (
				'q'    => $keyword,
				'tpr' => '1',
				'ts'   => time()*1000
		));
		...
		switch ($this->getOption ( 'scrape_source' )) {
			...
			default :
				if (preg_match_all ( '/<li.*?>.*?<span.*?>(.*?)<\/span>.*?<\/li>/is', $request->getBody (), $result, PREG_PATTERN_ORDER )) 
				{
