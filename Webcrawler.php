<?php
/**
 * Content scraper for http://www.webcrawler.com
 *
 * @author SSES Mike
 * @label  WebCrawler
 */
class Content_Adapter_Webcrawler extends Content_Adapter_DeepLinkingAbstract {
	/**
	 * Fetch content
	 *
	...
	public function fetchContent($keyword) {
		$this->getClient ()->setHeaders ( 'Referer', 'http://www.webcrawler.com/' )
			->setUri ( 'http://www.webcrawler.com/serp' )->setParameterGet ( array (
				'q'    => $keyword,
		));
		...
		switch ($this->getOption ( 'scrape_source' )) {
			...
			default :
				if (preg_match_all ( '/<div\\s*class="result">.*?<span>(.*?)<\/span>.*?<\/div>/is', $request->getBody (), $result, PREG_PATTERN_ORDER )) 
				{
