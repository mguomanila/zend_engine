<?php
/**
 * Content scraper for http://fr.ask.com
 *
 * @author BHC
 * @label  Ask
 */
class Content_Adapter_Ask extends Content_Adapter_DeepLinkingAbstract {
	/**
	 * Fetch content
	 *
	...
	public function fetchContent($keyword) {
		$this->getClient ()->setHeaders ( 'Referer', 'http://fr.ask.com/' )
			->setUri ( 'http://fr.ask.com/web' )->setParameterGet ( array (
				'q'    => $keyword,
				'qsrc' => '0',
				'o'    => '0',
				'l'    => 'dir',
		...
		switch ($this->getOption ( 'scrape_source' )) {
			...
			default :
				if (preg_match_all ( '/<div.*?class="PartialSearchResults-item".*?>.*?<p\\s*class="PartialSearchResults-item-abstract">(.*?)<\/p>.*?/is', $request->getBody (), $result, PREG_PATTERN_ORDER )) 
				{
